...
test("opens source information and the full transcript", async ({ page }) => {
  ...
  const source = page.getByRole("dialog", { name: "Итоговый" });
  await expect(source).toBeVisible();
  await expect(source.getByText("Выпуск №2 · 2015")).toBeVisible();
  await expect(source.getByRole("link", { name: "Открыть выпуск" })).toHaveAttribute(
    "href",
    "https://zavtracast.ru/02.html",
  );
  await source
  ...
  const transcript = page.getByRole("dialog", { name: "Итоговый" });
